...
export default function Page() {
  ...
  function filter_timeline(values) {
    setTimeline(values);
    setDispData(
      totalData.filter((d) => d.year >= values[0] && d.year <= values[1])
    );
  }
      ...
      <Grid container spacing={1}>
        ...
        <Grid item xs={3}>
          <MassChart
            data={dispData}
            selectedData={selectedData}
            setSelectedData={updateMapHover}
          />
          <ClassChart
            data={dispData}
            selectedData={selectedData}

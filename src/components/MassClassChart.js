  ...
  function get_color(d) {
    ...
    if (selectedData !== null && selectedData[0] !== undefined) {
      for (var i = 0; i < selectedData.length; i++) {
        ...
        if (
          selectedData[i].mass / 1000 >= masses[0] &&
          selectedData[i].mass / 1000 < masses[1] &&
          mod_cls === cls
        ) {
          return "#D55D0E";
        }
...
export default function MassClassChart(props) {
  ...
  for (var i = 0; i < all_classes.length; i++) {
    ...
    cls_masses[cls] = {
      "<r1":
        subset_masses.filter((m) => {
          return m / 1000 < range1;
        }).length / total_masses,
      "r1-r2":
        subset_masses.filter((m) => {
          return m / 1000 >= range1 && m / 1000 < range2;
        }).length / total_masses,
      "r2-r3":
        subset_masses.filter((m) => {
          return m / 1000 >= range2 && m / 1000 < range3;
        }).length / total_masses,
      "r3-r4":
        subset_masses.filter((m) => {
          return m / 1000 >= range3 && m / 1000 < range4;
        }).length / total_masses,
      "r4-r5":
        subset_masses.filter((m) => {
          return m / 1000 >= range4 && m / 1000 < range5;
        }).length / total_masses,
      "r5-r6":
        subset_masses.filter((m) => {
          return m / 1000 >= range5 && m / 1000 < range6;
        }).length / total_masses,
      "r6-r7":
        subset_masses.filter((m) => {
          return m / 1000 >= range6 && m / 1000 < range7;
        }).length / total_masses,
      "r7-r8":
        subset_masses.filter((m) => {
          return m / 1000 >= range7;
        }).length / total_masses,
    };
  ...
  for (var j = 0; j < uniq_classes.length; j++) {
    ...
    var tmp = data.filter((d) => {
      return d.class === uniq_classes[iter];
    });
    class_data.push({
  ...
  class_data = class_data.sort((a, b) => {
  ...
  var keysSorted = class_data.map((d) => {
    return d["class"];
  });
  ...
  var to_display = top_X.reduce((acc, curr) => {
    ...
    acc[curr] = Object.fromEntries(
      entries.map((curr, index) => {
        entries[index][1] =
          index === 0 ? curr[1] : entries[index - 1][1] + curr[1];
        return [curr[0], entries[index][1]];
      })
    );
    return acc;
  }, {});

  var x = scaleLinear().domain([0, 1]).range([0, width]);
  ...
  function setGlobal(range, cls) {
    ...
    var tmp = data.filter((d) => {
      return (
        d.mass / 1000 >= lower &&
        d.mass / 1000 < upper &&
        (keysSorted.includes(process(d.class)) ? process(d.class) : "other") ===
          cls
      );
    });
    setSelectedData(tmp);
      ...
      <svg className={classes.svg} id="MassClassChart">
        <g transform={`translate(${margin.left}, ${margin.top})`}>
          <text
            x={-margin.left / 1.2}
            y={-vh(5)}
            style={{ fill: "#4fbbd6", fontSize: "15px" }}
          >
            Kilograms
          </text>
          <text
            x={-margin.left / 1.2 + vw(14)}
            y={-vh(5)}
            style={{ fill: "#4fbbd6", fontSize: "15px" }}
          >
            # Classes:{" "}
          ...
          <g
            transform={`translate(${-margin.left + vw(0.5)}, ${
              -margin.top / 2
            })`}
            ref={(node) =>
              select(node)
                .call(axisBottom(legend_axis).tickSize(0))
                .select(".domain")
                .remove()
            }
          >
          ...
          <g
            ...
            ref={(node) =>
              select(node).call(axisBottom(x).ticks(width / 50, "%"))
            }
          />

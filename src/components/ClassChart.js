...
export default function ClassChart(props) {
  ...
  var color = scaleOrdinal().domain(key_colors).range(scheme);

  var make_pie = pie().value((d) => {
  ...
  function ToolTip() {
    ...
    if (hover !== null) {
      ...
      value = data_ready.filter((d) => {
        return d.data.class === key;
      })[0].value;
    } else {
      return <div />;
    }
  ...
  return (
    <Paper className={classes.paper}>
      <svg className={classes.svg} id="ClassChart">
        ...
        <g
          transform={`translate(${width / 2 + radius / 16}, ${
            height / 2 + radius / 4 + margin.top
          })`}
          key={"pie_chart"}
        >
          {data_ready.map((d, i) => {

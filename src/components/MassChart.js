...
export default function MassChart(props) {
  ...
  for (var i = 0; i < breakpoints.length; i++) {
    ...
    if (i === 0) {
      ...
      range = [breakpoints[i - 1], Number.MAX_SAFE_INTEGER];
    } else {
      range = [breakpoints[i - 1], breakpoints[i]];
    }
    const r = range;
    var tmp = data.filter((d) => {
      return d.mass / 1000 >= r[0] && d.mass / 1000 < r[1];
    });
    mass_data.push({
      range: r,
      data: tmp,
  ...
  for (var j = 0; j < breakpoints.length; j++) {
    if (j === 0) {
      range_strings.push(`<${breakpoints[0]}`);
    } else if (j === breakpoints.length - 1) {
      range_strings.push(`${breakpoints[j - 1]}+`);
    } else {
      range_strings.push(`${breakpoints[j - 1]}-${breakpoints[j]}`);
    }
  }

  var x = scaleBand().domain(breakpoints).range([0, width]);

  var labels = scaleBand().domain(range_strings).range([0, width]);
  ...
  var y = scaleLinear().range([height, 0]).domain([0, y_max]);
  ...
  function ToolTip() {
    ...
    if (min < breakpoints[0]) {
      name = "<" + max;
    } else if (max === breakpoints[breakpoints.length - 1]) {
      name = min + "+";
    } else {
      name = min.toString() + "-" + max;
    }
    return (
      <text
      ...
      <svg className={classes.svg}>
        <g transform={`translate(${margin.left}, ${margin.top})`}>
          <g
            ...
            ref={(node) => select(node).call(axisBottom(labels))}
          />
          <g>
